...
docker build -t husarion/rviz:latest -f Dockerfile --build-arg RAND=$RANDOM --build-arg BUILD_TYPE=$build_type .


# Get current date in YYYY-MM-DD format
printf -v date '%(%Y-%m-%d)T' -1
# Generate tag
auto_tag="husarion/rviz:$tag_prefix-$date$tag_postfix"
echo "Adding tag: $auto_tag"

docker tag husarion/rviz:latest  $auto_tag
